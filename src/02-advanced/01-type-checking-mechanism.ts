...
/**
 * 当 y 可以给 x 赋值时, 即 x = y, 我们说 x 兼容 y 类型
...
/**
 * 类型兼容性: 函数
 * a. 函数参数的个数, 参数多的函数兼容参数少的函数
 *   个人理解: 假如下方 hof 函数的参数 h 是求两数之积,
 *     h1 函数传入一个数字, 结果会报错, 此时可以通过报错排查到问题
 *     h3 函数传入三个数字 , 结果不会报错, 但是返回的数据是其中两个数字的积, 对于传入 h3 的使用者来说, 不符合预期, 因为他想计算的是三数之积, 这种问题不好排查
 *
 */
